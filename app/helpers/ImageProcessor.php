...
class ImageProcessor {
	...
	public static function resize($imagePath,$opts=null){
		// Yii::log('resizing ' . $imagePath);
		$imagePath = urldecode($imagePath);
		# start configuration
		$cacheFolder = app()->config('imagecachesPath'); # path to your cache folder, must be writeable by web server
		$remoteFolder = $cacheFolder.'remote/'; # path to the folder you wish to download remote images into

		$defaults = array('curl' => false, 'crop' => false, 'scale' => 'false', 'thumbnail' => false, 'maxOnly' => true,
		...
		$ext = empty($finfo['extension']) ? 'jpg' : $finfo['extension'];
		...
		if(isset($purl['scheme']) && ($purl['scheme'] == 'http' || $purl['scheme'] == 'https')):
			...
			if($download_image == true):
				...
				try {
					if ($opts['curl'] == true) {
						// Yii::log("curl is true");
						if (!empty($opts['maxCurlSize'])) {
							// Yii::log("max curl size is " . $opts['maxCurlSize']);
							if (ImageProcessor::grabSize($imagePath) <= ($opts['maxCurlSize']*1024*1024)) {
								// Yii::log("grab the image $imagePath and download it to $local_filepath");
					...
					if (empty($img)) {
						// Yii::log("img is empty");
						// Yii::log("ERROR: Tried to get image $imagePath with opts['curl'] not turned on");
					...
					errorLog("ERROR: While doing image resize, got this error: ".$e->getMessage());
				}
			endif;
		...
		if($create == true):
			...
			if($return_code != 0) {
				errorLog("Tried to execute : $cmd, return code: $return_code, output: " . print_r($output, true));
				return false;
			}
		endif;
	...
	public static function grabSize($remoteFile) {
		...
		// Yii::log("contentLength is $contentLength");
